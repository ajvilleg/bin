...
<body>
<br>
<h1>Ecoli O83 AutoFACT Index</h1>
<br>
...
while(<SIZES>) {
	...
	if (-e "$i/$i.orfs.out") { # if the orfs.out file exists, i.e. if there are orfs
		...
		$pwd = `pwd`;
		print $pwd;
		system("cp /home/ajvilleg/PostAutoFACTScripts/AF2EMBL.pl .") && die "Failed: error code = $?\n";
		system("/usr/bin/perl AF2EMBL.pl $i.predict $i.orfs.out $i.fas > $i.embl") && die "Failed: error code = $?\n";
		...
		<jnlp
			...
			<application-desc main-class="uk.ac.sanger.artemis.components.ArtemisMain">
				<argument>http://rita.imb.nrc.ca/~villegas/AnnotationDone/091709/$i/$i.embl</argument>
			</application-desc>
       	 	</jnlp>	
ENDJNLP
...
system("cp /home/ajvilleg/PostAutoFACTScripts/changeNCBIScoreBarLink_helios.csh .") && die "Failed: error code = $?\n";
system("./changeNCBIScoreBarLink_helios.csh");
unlink("changeNCBIScoreBarLink_helios.csh");

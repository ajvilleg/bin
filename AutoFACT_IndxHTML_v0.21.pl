...
# 
# USAGE: perl AutoFACT_IndxHTML [name of organism]
# History
# v0.2	Oct 5 2009	- Added some bioperl code to do the EMBL-)GBK conversion instead of readseq
# v0.21 Oct 28 2009	- Removed Artemis link via JNLP. Added powmap.jar from artemis_on_the_web as link.
#	Nov 30 2009	- Added code to copy over powmap.jar from ~/bin 
use subs 'println';
use Bio::SeqIO;
require "/home/avillegas/bin/println.pl";
...
print "Printing index.html headers...";
...
print "Sorting seqsizes by decreasing BP size...";
...
while(<SIZES>) {
	...
	if (-e "$i/$i.orfs.out") { # if the orfs.out file exists, i.e. if there are orfs
		...
		print "Running AF2EMBL.pl on $pwd...";
		system("cp ~/bin/AF2EMBL.pl .") && die "cp ~/bin/AF2EMBL.pl . failed: error code = $?\n";
		system("/usr/bin/perl AF2EMBL.pl $i.predict $i.orfs.out $i.fas > $i.embl") && die "/usr/bin/perl AF2EMBL.pl $i.predict $i.orfs.out $i.fas > $i.embl failed: error code = $?\n";
		system("chmod 644 $i.embl") && die "chmod 644 $i.embl failed: error code = $?\n";
		# Remove the script from the current directory to cleanup
		system("rm AF2EMBL.pl") && die "rm AF2EMBL.pl failed: error code = $?\n";
		println "DONE";
		...
		print "Converting $i EMBL file to GBK using ";
		$style = "java"; # or bioperl
		
		if ($style eq "java") {
			print "JAVA...";
			system("cp ~/bin/readseq.jar .") && die "cp ~/bin/readseq.jar . failed: error code = $?\n";
			system("./readseq.jar -f Genbank -o $i.gbk $i.embl") && die "Failed: error code = $?\n";
			system("rm readseq.jar") && die "rm readseq.jar failed: error code = $?\n";
		}
		elsif ($style eq "bioperl") {
			print "BIOPERL...";
			$inFile = $i .".embl";
			$outFile = $i . ".gbk";
		...
		#		<argument>http://76.70.11.198/annot/091709/$i/$i.embl</argument>
		#	</application-desc>
       	 	#</jnlp>	
#ENDJNLP
# the string terminator ENDJNLP above should be unquoted and not surrounded by any whitespace so it'll work
		...
		print "Printing html links for $i...";
		...
		print "NO ORFS for $i, printing stuff...";
		# print contig name
		print INDEX "&nbsp;&nbsp;&nbsp;&nbsp;<i>$i</i>\n";
...
# Applet for viewing the genome via Artemis
print "Printing Applet code...";
system("cp ~/bin/artemis_on_the_web/powmap.jar .") && die "Failed: error code = $?\n";
print INDEX "<applet code='DianaApplet.class' archive='powmap.jar' width=1500 height=150><param name=entries value='$powmapvals'>";
println "DONE";
...
system("cp ~/bin/changeNCBIScoreBarLink_thunder.csh .") && die "Failed: error code = $?\n";
system("./changeNCBIScoreBarLink_thunder.csh");
unlink("changeNCBIScoreBarLink_thunder.csh");
println "DONE";

...
#
# USAGE : AF2EMBL [glimmer *.predict file] [AutoFACT *.out file] [fasta file *.fna] [tRNAscan_SE output file *.tRNA] > (output EMBL file) 
#
# History:
#
...
# v0.3	3 Dec 2009		- added mol_type tag under source
#				- changed $gene to $locus_tag
#				- changed @results to @predict to more reflect that data is coming from Glimmer output
#				- Added $gene names by assigning locus names from AutoFACT which have 3 lowercase letters than 1 uppercase
#				- Added $EC_number if it's there
#				- Added the gene main tags to go along with the CDS main tag
# v0.31 4 Aug 2010		- Added commas after -complete => 1, and -throw => 1, They weren't executing properly because syntax was wrong.
#				  script supposed to die if CDS is improper format.
#				- Added -id => $locus_tag so that error location can be identified.
#
###################################################################################
use Bio::Seq;
use Bio::SeqIO;
...
my $FNAfile = $ARGV[2];
my $tRNAScan_out = $ARGV[3];
parse_glimmer($Glimmer3_output, $AutoFACT_output, $FNAfile);
#parse_glimmer($Glimmer3_output, $AutoFACT_output, $FNAfile, $tRNAScan_out);
...
my $tRNA;		#filehandle for tRNAScan-SE input
#my $output;		#filehandle for output
my @predict;		#predict results array from Glimmer
...
while(<GLIMMER>){
	if(/>/) { #checks if the character > is in the current line
		...
		until ($currAnn[0] eq $locus_tag && $currAnn[1] eq "AutoFACT") {
			$inx++;
			if ($inx > $size-1) { exit 0; } #we are past the end of file
			@currAnn = split(/\t/, $autoF[$inx]);
		}
		
		#### Cleaning up product name ####
		# added to remove 'Cluster: ' clause in note
                ...
                my $product = "";
                if ($nPos<0) { # if "n=" substring is not there, just use the whole thing
                        $product = $currAnn[4];
                }
                else {
                        $product = substr($currAnn[4], 0, $nPos-1); # otherwise have to extract the proper string
                }
		...
		if($end < $start) {
			print("FT   gene            complement($end..$start)\n");
			print("FT                   \/locus_tag=\"$locus_tag\"\n");
			if ($gene ne "") {
        	                print("FT                   \/gene=\"$gene\"\n");
	                }
			...
			my $dna_obj = Bio::Seq->new(-seq => $dna,
						    -id => $locus_tag,	# Added so that when warnings thrown, location of error can be identified.
						   );
			...
			$trans = $dna_obj->translate(-codontable_id => $transl_table, # Bacterial codon table
									  -complete => 1,	# Translate CDS the way EMBL/GenBank/DDBJ do it, 
												# confirms start and terminator codonsand no terminators 
												# within the coding region. Also converts initial amino acid 
												# to Methionine if it's non-ATG
									  -throw => 1,		# Die if improper CDS after it checks above since -complete is set to true
									);
